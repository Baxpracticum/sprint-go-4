...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedInMps := action / mInKm // скорость в м/с
	return ((walkingCaloriesWeightMultiplier * weight) + ((math.Pow(float64(speedInMps), 2) / height) * walkingSpeedHeightMultiplier * weight)) * duration * minInH
}
