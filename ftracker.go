...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	var averageSpeedInSec float64 = math.Pow(meanSpeed(action, duration)*kmhInMsec, 2) // скорость в м/с
	return ((walkingCaloriesWeightMultiplier*weight + (averageSpeedInSec/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
